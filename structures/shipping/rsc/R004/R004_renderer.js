// ============================================================
// R004_renderer.js - A-Type RSC Shipping Box 4 with handle cuts
// Depends on: R004_spec.js, R004_layout.js
// ============================================================

function R004_renderSVG(cfg, appState) {
  var layout = R004_getLayout(cfg.W, cfg.D, cfg.H, cfg);
  var pad = 80;
  ...
  svg += '  <defs>\n';
  svg += R004_dimensionArrowMarkerDef();
  ...
  if ((!appState || appState.showPerforation) && cfg.handleHoleEnabled !== false) {
    svg += '    <g id="layer-perforation">\n';
    layout.handleCutPaths.forEach(function(d) {
      svg += '      <path class="thomson" d="' + d + '"/>\n';
    });
    svg += '    </g>\n';
  }
...
function R004_buildExportSVG(cfg) {
  var layout = R004_getLayout(cfg.W, cfg.D, cfg.H, cfg);
  var pad = 5;
  var vbX = layout.bounds.minX - pad;
  var vbY = layout.bounds.minY - pad;
  ...
  out += '  </g>\n';
  if (cfg.handleHoleEnabled !== false) {
...
function R004_buildDXF() {
  return '';
}

function R004_dimensionArrowMarkerDef() {
