...
function B001_mapPoint(x, y, ctx) {
  return {
    x: B001_layoutNum(B001_mapX(Number(x), ctx)),
...
function B001_getLayout(W, D, H) {
  ...
  const perforationElements = B001_CUT_ELEMENTS
    .filter(B001_isPerforationSourceElement)
    .map(el => B001_transformElement(el, ctx));
  const foldElements = B001_FOLD_ELEMENTS.map(el => B001_transformElement(el, ctx));
  const bleedElements = B001_BLEED_ELEMENTS.map(el => B001_transformElement(el, ctx));
  const guideElements = B001_GUIDE_ELEMENTS.map(el => B001_transformElement(el, ctx));
  const bounds = B001_getBoundsFromElements([cutElements, perforationElements, foldElements, bleedElements]);
