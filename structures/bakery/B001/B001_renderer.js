...
function B001_buildDimensionLayer(cfg, layout) {
  ...
  if (!a) return '';
  const x = a.panelsX;
  const dimY = a.bodyTopY + (a.bodyBottomY - a.bodyTopY) * 0.66;
  const dimX = x[1] - Math.min(18, cfg.W * 0.18);

  function hDim(x1, x2, y, label) {
    const mid = (x1 + x2) / 2;
    const gap = Math.min(18, Math.max(8, (x2 - x1) * 0.16));
    return '<line x1="' + B001_num(x1 + 2) + '" y1="' + B001_num(y) + '" x2="' + B001_num(mid - gap) + '" y2="' + B001_num(y) + '" stroke="#111" stroke-width="0.35" marker-start="url(#arrow)"/>' +
      '<line x1="' + B001_num(x2 - 2) + '" y1="' + B001_num(y) + '" x2="' + B001_num(mid + gap) + '" y2="' + B001_num(y) + '" stroke="#111" stroke-width="0.35" marker-start="url(#arrow)"/>' +
      '<text class="dim" x="' + B001_num(mid) + '" y="' + B001_num(y + 3.3) + '" font-size="5.2" text-anchor="middle">' + label + '</text>';
  }

  function vDim(xPos, y1, y2, label) {
    const mid = (y1 + y2) / 2;
    const gap = Math.min(18, Math.max(8, (y2 - y1) * 0.16));
    const textX = xPos + 4.5;
    return '<line x1="' + B001_num(xPos) + '" y1="' + B001_num(y1 + 2) + '" x2="' + B001_num(xPos) + '" y2="' + B001_num(mid - gap) + '" stroke="#111" stroke-width="0.35" marker-start="url(#arrow)"/>' +
      '<line x1="' + B001_num(xPos) + '" y1="' + B001_num(y2 - 2) + '" x2="' + B001_num(xPos) + '" y2="' + B001_num(mid + gap) + '" stroke="#111" stroke-width="0.35" marker-start="url(#arrow)"/>' +
  ...
  out += hDim(x[0], x[1], dimY, 'W ' + cfg.W + 'mm');
  out += hDim(x[1], x[2], dimY, 'D ' + cfg.D + 'mm');
  out += hDim(x[2], x[3], dimY, 'W ' + cfg.W + 'mm');
  out += hDim(x[3], x[4], dimY, 'D ' + cfg.D + 'mm');
  out += vDim(dimX, a.bodyTopY, a.bodyBottomY, 'H ' + cfg.H + 'mm');
  out += '  </g>\n';
  return out;
}
...
function B001_renderSVG(cfg, appState) {
  ...
  svg += '  <g id="layer-panel-fill" transform="' + matrix + '">' +
    layout.bleedElements.map(el => B001_restyleElement(el, 'panel-fill')).join('') + '</g>\n';
  ...
  if (!appState || appState.showCut) {
    svg += '  <g id="layer-cut" transform="' + matrix + '">' +
      layout.cutElements.map(el => B001_restyleElement(el, 'cut-fill')).join('') + '</g>\n';
  }
...
function B001_buildPDF(cfg) {
  ...
  const content = [
    ...
    B001_pdfLineOps(layout.cutElements, layout, bounds, pad, '0.8 0 0 RG'),
