...
function M002_handleHolePaths(layout, cfg) {
  ...
  var width = Number(cfg && cfg.handleHoleWidth) || 25;
  var height = Number(cfg && cfg.handleHoleHeight) || 25;
  var radius = Math.min(width, height) / 2;
  var sourceCenters = [
    { id: 'handleHoleLeft', cx: (785.248 + 856.114) / 2, cy: (1280.264 + 1351.13) / 2 },
    { id: 'handleHoleRight', cx: (1397.532 + 1468.398) / 2, cy: (1280.264 + 1351.13) / 2 }
  ];

  return sourceCenters.map(function(hole) {
    var cx = hole.cx * t.a + t.e;
    var cy = hole.cy * t.d + t.f;
    return {
      id: hole.id,
      d: M002_roundRectPath(cx - width / 2, cy - height / 2, width, height, radius)
    };
  });
}
