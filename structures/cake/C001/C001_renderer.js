...
function C001_buildDimensionLayer(cfg, layout) {
  function hDim(x1, x2, y, label, textDy) {
    const p1 = C001_map(layout, x1, y);
    const p2 = C001_map(layout, x2, y);
    const midX = (p1.x + p2.x) / 2;
    const gap = Math.min(52, Math.max(22, Math.abs(p2.x - p1.x) * 0.18));
    const dir = p2.x >= p1.x ? 1 : -1;
    const arrowLen = 5;
    const arrowHalf = 3;
    const trim = Math.min(arrowLen, Math.abs(p2.x - p1.x) * 0.08);
    const leftLineStart = C001_num(p1.x + (dir * trim));
    const leftLineEnd = C001_num(midX - (dir * gap));
    const rightLineStart = C001_num(p2.x - (dir * trim));
    const rightLineEnd = C001_num(midX + (dir * gap));
    const arrow1 = [
      C001_num(p1.x), C001_num(p1.y),
      C001_num(p1.x + (dir * arrowLen)), C001_num(p1.y - arrowHalf),
      C001_num(p1.x + (dir * arrowLen)), C001_num(p1.y + arrowHalf)
    ].join(' ');
    const arrow2 = [
      C001_num(p2.x), C001_num(p2.y),
      C001_num(p2.x - (dir * arrowLen)), C001_num(p2.y - arrowHalf),
      C001_num(p2.x - (dir * arrowLen)), C001_num(p2.y + arrowHalf)
    ].join(' ');
    return '<line x1="' + leftLineStart + '" y1="' + p1.y + '" x2="' + leftLineEnd + '" y2="' + p1.y + '" stroke="#111" stroke-width="0.35"/>' +
      '<line x1="' + rightLineStart + '" y1="' + p2.y + '" x2="' + rightLineEnd + '" y2="' + p2.y + '" stroke="#111" stroke-width="0.35"/>' +
      '<polygon points="' + arrow1 + '" fill="#111"/>' +
      '<polygon points="' + arrow2 + '" fill="#111"/>' +
      '<text class="dim" x="' + C001_num(midX) + '" y="' + C001_num(p1.y + textDy) + '" font-size="6.4" text-anchor="middle">' + label + '</text>';
  }
  ...
  function vDim(x, y1, y2, label) {
    ...
    const gap = Math.min(48, Math.max(22, Math.abs(p2.y - p1.y) * 0.18));
    const textX = p1.x + 8;
    return '<line x1="' + p1.x + '" y1="' + p1.y + '" x2="' + p1.x + '" y2="' + C001_num(midY - gap) + '" stroke="#111" stroke-width="0.35" marker-start="url(#arrow)"/>' +
      '<line x1="' + p2.x + '" y1="' + p2.y + '" x2="' + p2.x + '" y2="' + C001_num(midY + gap) + '" stroke="#111" stroke-width="0.35" marker-start="url(#arrow)"/>' +
      '<text class="dim" x="' + C001_num(textX) + '" y="' + C001_num(midY) + '" font-size="6.4" transform="rotate(-90 ' + C001_num(textX) + ' ' + C001_num(midY) + ')" text-anchor="middle">W ' + label + 'mm</text>';
  }

  let out = '  <g id="layer-dimensions">\n';
  out += hDim(460.09, 1239.618, 1002, 'D ' + cfg.D + 'mm', 4.8);
  out += vDim(1091, 483, 1269, cfg.W);
  out += hDim(1239.618, 1636.469, 1002, 'H ' + cfg.H + 'mm', 4.8);
  out += '  </g>\n';
  return out;
}
...
function C001_renderSVG(cfg, appState) {
  ...
  if (!appState || appState.showDims) svg += C001_buildDimensionLayer(layout.cfg, layout);
