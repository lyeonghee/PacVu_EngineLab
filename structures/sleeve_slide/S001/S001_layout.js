...
function S001_getInsertSpec(innerSpec, clearance, cfg) {
  ...
  const W = Math.max(20, innerSpec.W - 16 * structureScale - gapDelta * 2);
  const D = Math.max(4, innerSpec.D - 27 * structureScale - gapDelta);
  const H = Math.max(20, innerSpec.H - 38 * structureScale - gapDelta * 2);
...
function S001_arrangeParts(sourceParts, cfg) {
  ...
  if (mode === 'outerSleeve') {
    ...
    if (c.showOuterSleeve !== false) parts.push(outer);
    if (c.showInnerTray !== false) parts.push(inner);
    if (c.insertPadEnabled !== false && c.showInsertPad !== false) parts.push(insert);
  }
