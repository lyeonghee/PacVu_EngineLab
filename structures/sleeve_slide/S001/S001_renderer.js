...
function S001_partDimensionGuides(part) {
  ...
  const labelW = 'W ' + S001_dimensionValue(W) + 'mm';
  const labelD = 'D ' + S001_dimensionValue(D) + 'mm';
  const labelH = 'H ' + S001_dimensionValue(H) + 'mm';
  const inset = 22 * k;
  const topGap = 24 * k;
  const sideGap = 26 * k;
  ...
  if (part.key === 'outerSleeve' && x.length >= 5 && y.length >= 4) {
    ...
    const labelFrontH = 'H ' + S001_dimensionValue(frontH) + 'mm';
    return [
