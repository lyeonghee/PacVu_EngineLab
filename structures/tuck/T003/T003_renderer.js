...
function T003_holeDefs(layout, cfg) {
  if (cfg && cfg.stringHoleEnabled === false) return [];
  ...
  return holes.map(hole => {
    ...
    const dia = T003_getHoleValue(cfg, 'holeDia', defaultDia);
    return {
      id: hole.id,
      cx: hole.cx * t.a,
      cy: hole.cy * t.d,
...
function T003_buildDimensionLayer(cfg, t) {
  ...
  return '  <g id="layer-dimensions">' +
    line(tx(315.482), ty(874.418), tx(560.679), ty(874.418), 'W ' + cfg.W + 'mm') +
    line(tx(560.679), ty(874.301), tx(805.876), ty(874.301), 'D ' + cfg.D + 'mm') +
    vline(tx(506.466), ty(377.008), ty(1216.063), 'H ' + cfg.H + 'mm') +
    '</g>\n';
}
